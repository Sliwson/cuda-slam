...
\label{sec:theory}
\textbf{Simultaneous localization and mapping} (in short SLAM) is the computational problem of constructing or updating a map of an unknown environment while simultaneously keeping track of an agent's location within it \cite{ms-wiki1}. In theory the problem can be simplified to more mathematical definition - given two point sets $P_1 = \{x_1,\dots, x_n\}$ and $P_2=\{y_1, \dots, y_n\}$ find translation $t$ and rotation $R$ that minimizes the mean squared error:
...
\item Compute the mean squared error.
...
\begin{enumerate}
\setcounter{enumi}{5}
\item To increase convergence ratio, repeat those steps for multiple scans of observed object and select the transformation minimizing the mean squared error.
\end{enumerate}
...
Secondly the main bottleneck of the algorithm is the calculation of matrix-vector products $P1$, $P^T1$ and $PX$. This is done with $O(MN)$ complexity. The Fast Gauss Transform, introduced by Greengard and Strain \cite{fgt}, reduces complexity to only $O(M+N)$.
...
\item \textit{cpu-slam} - executable project, contains implementation of slam methods for CPU and CPU entry point.
\item \textit{gpu-slam} - executable project, contains implementation of slam methods for GPU and GPU entry point.
\end{itemize}
...
\item \textit{Algorithms} - implemented in \textit{cuda-slam} as well as in \textit{cpu-slam}. Each algorithm is implemented in a separate file as a global function in appropriate namespace. Some operations that are common for CPU as well as GPU are implemented in \textit{common} project.
\end{itemize}
...
\item \textit{cuBLAS} and \textit{cuSOLVER} - libraries created and natively supported by Nvidia for GPU accelerated matrix operations using CUDA technology.
...
\item \textbf{None} - after each repetition, error is calculated by finding correspondences between subcloud of given size taken from the first cloud and the entire second cloud. This method offers higher reliability but instead increases the execution time of one iteration noticeably, especially using CPU.
...
CPU solution of a Coherent Point Drift is based on algorithm described in \cite{cpd}. It also uses Matlab code provided by authors as an example of how some technical details were handled. FGT algorithm implementation is based on Matlab sample available in \cite{fgtmatlab}.
Parallelised solution using GPU acceleration mainly focuses on matrices multiplication using cuBLAS and utilizing thrust. FGT algorithm was not implemented on GPU due to complexity of this solution and lack of reliable external sources.
...
\item One parameter (config file name) - load config from the given file.
\begin{figure}[H]
...
As we can see, the benchmarks prove the expectations and parallelisation of the ICP algorithm results in big performance improvements. Having iteration time lower than $100\:ms$ for clouds with size of $100000$ points more tests could be run for the GPU implementation.
...
For the performance tests \textit{cpd-weight} parameter was set to $0.1$ and \textit{const-scale} to $false$. The other parameters were the same as in the other benchmarks. We present the results using logarithmic scale on y axis. Similarly to Non-iterative Closest Point benchmarks we compare three approximation types, this time approximations relying on using Fast Gauss Transform.
...
We tested our methods using the clouds and conditions that may be closer to reality. That is, we used clouds that are missing points, even large part of them, and we corrupted clouds by noise and outliers. For this purpose we have used CPU versions of algorithms. For ICP and NICP \textit{policy} was set to \textit{parallel} and \textit{approximation-type} to \textit{None}. For CPD \textit{approximation-type} was set to \textit{Hybrid}, mainly to achieve lower computation times. Many tests were conducted and it turned out that the results are highly dependent on configuration. Therefore, in this section you can find the most interesting results and tests that have shown the most consistent characteristics. All the results are available in spreadsheet kept on Github: \href{https://github.com/Sliwson/cuda-slam/tree/master/doc/noise}{https://github.com/Sliwson/cuda-slam/tree/master/doc/noise}. All the test configurations are to be found in a subdirectory \textit{configs}. \\
After testing, we came to several conclusions. Firstly, the ICP algorithm gave the worst results for almost all the test sets. The main reason could be cloud sizes used in tests, which occurred to be often too big for ICP capabilities. Algorithm had also trouble with noise. Even low noise level such as \textit{noise-affected-points-before} equal to $0.1$ and \textit{noise-intensity-before} equal to $0.05$ may cause convergence failure. Secondly, increasing \textit{max-iterations} can help achieving more exact results.\\
CPD algorithm mostly achieved correct results. Main key to CPD convergence is to select parameters value correctly. The most important one is \textit{cpd-weight}. Difference of $0.3$ can lead to significantly different result. Second CPD parameter is \textit{cpd-const-scale}. Setting it to \textit{true} may help to achieve better results when we know that the scale did not change after transformation. Especially it helps when clouds contain a lot of noise and outliers. Also, decreasing \textit{cpd-tolerance} and \textit{converge-epsilon} may result in increasing iterations count and lead to more exact results.\\
All the algorithms had tendency to match areas where the points density was the highest. An example of this behaviour may be observed using \textit{config21.json}. It uses two "airbus" clouds with decreased sizes and added noise and outliers. Result achieved by algorithms are presented below and vary among each other.

\begin{figure}[H]
	\centering
    \subfloat[\centering CPD]{{\includegraphics[width=5cm]{mr-cpd-21.png} }}
    \hspace{0.5em}
    \subfloat[\centering ICP]{{\includegraphics[width=5cm]{mr-icp-21.png} }}
    \hspace{0.5em}
    \subfloat[\centering NICP]{{\includegraphics[width=5cm]{mr-nicp-21.png} }}
...
Similar situation was met using \textit{config11.json} and \textit{config18.json}. Unfortunately in tests using the latter one, ICP returned identity matrix. The results are presented below.

\begin{figure}[H]
	\centering
    \subfloat[\centering CPD]{{\includegraphics[width=5cm]{mr-cpd-11.png} }}
    \hspace{0.5em}
    \subfloat[\centering ICP]{{\includegraphics[width=5cm]{mr-icp-11.png} }}
    \hspace{0.5em}
    \subfloat[\centering NICP]{{\includegraphics[width=5cm]{mr-nicp-11.png} }}
    \caption{Tests with \textit{config11.json}}
\end{figure}

\begin{figure}[H]
	\centering
    \subfloat[\centering CPD]{{\includegraphics[width=6cm]{mr-cpd-18.png} }}
    \hspace{0.5em}
    \subfloat[\centering NICP]{{\includegraphics[width=6cm]{mr-nicp-18.png} }}
    \caption{Tests with \textit{config18.json}}
\end{figure}

In contrary to \textit{config18.json} and \textit{config21.json}, using \textit{config11.json} CPD found almost exact solution while this time NICP focused on matching areas with the highest density.

Another example of CPD accuracy in tough conditions can be observed when using \textit{config26.json}. We used two "airbus" clouds where first had no wheels, engines and a vertical stabilizer. The second one had wings and wheels cut. Also we have added noise and outliers to both clouds. Results are presented below.

\begin{figure}[H]
	\centering
    \subfloat[\centering CPD]{{\includegraphics[width=5cm]{mr-cpd-26.png} }}
    \hspace{0.5em}
    \subfloat[\centering ICP]{{\includegraphics[width=5cm]{mr-icp-26.png} }}
    \hspace{0.5em}
    \subfloat[\centering NICP]{{\includegraphics[width=5cm]{mr-nicp-26.png} }}
    \caption{Tests with \textit{config26.json}}
\end{figure}

CPD found the exact solution. As mentioned above, ICP has troubles with incomplete clouds and it remains unchanged with this configuration. NICP was close to finding exact solution; however, it lacks of $90\degree$ rotation.

Finally, we prepared a test that focuses only on a situation where one cloud contains noise. We increased the \textit{noise-intensity} by small values and compared the results. The best method occurred to be NICP, CPD was the second and ICP was the third one. For those tests, the configurations \textit{config28.json} to \textit{config38.json} were used. Results are presented below.

\begin{figure}[H]
	\centering
    \subfloat[\centering CPD]{{\includegraphics[width=5cm]{mr-cpd-29.png} }}
    \hspace{0.5em}
    \subfloat[\centering ICP]{{\includegraphics[width=5cm]{mr-icp-29.png} }}
    \hspace{0.5em}
    \subfloat[\centering NICP]{{\includegraphics[width=5cm]{mr-nicp-29.png} }}
    \caption{Tests with \textit{config29.json}}
\end{figure}

\begin{figure}[H]
	\centering
    \subfloat[\centering CPD]{{\includegraphics[width=5cm]{mr-cpd-30.png} }}
    \hspace{0.5em}
    \subfloat[\centering ICP]{{\includegraphics[width=5cm]{mr-icp-30.png} }}
    \hspace{0.5em}
    \subfloat[\centering NICP]{{\includegraphics[width=5cm]{mr-nicp-30.png} }}
    \caption{Tests with \textit{config30.json}}
\end{figure}

\begin{figure}[H]
	\centering
    \subfloat[\centering CPD]{{\includegraphics[width=5cm]{mr-cpd-34.png} }}
    \hspace{0.5em}
    \subfloat[\centering ICP]{{\includegraphics[width=5cm]{mr-icp-34.png} }}
    \hspace{0.5em}
    \subfloat[\centering NICP]{{\includegraphics[width=5cm]{mr-nicp-34.png} }}
    \caption{Tests with \textit{config34.json}}
\end{figure}

\begin{figure}[H]
	\centering
    \subfloat[\centering CPD]{{\includegraphics[width=5cm]{mr-cpd-38.png} }}
    \hspace{0.5em}
    \subfloat[\centering ICP]{{\includegraphics[width=5cm]{mr-icp-38.png} }}
    \hspace{0.5em}
    \subfloat[\centering NICP]{{\includegraphics[width=5cm]{mr-nicp-38.png} }}
    \caption{Tests with \textit{config38.json}}
\end{figure}

NICP managed to find the exact solution in all the test cases. Results obtained by CPD were slightly worse and sometimes correction of \textit{cpd-weight} parameter was needed. ICP results started to differ even when \textit{noise-intensity} was still very small, thus its usage when conditions are not perfect is very limited.
...
\caption{Convergence rates for a Coherent Point Drift depending on cloud size}
\end{figure}
...
\caption{Convergence rates for a Coherent Point Drift depending on translation}
\end{figure}
Modifying a rotation of target cloud, the phenomenon described above seems to completely disappear. What is more, larger rotation results in higher convergence rate of tested algorithm but differences between them are significantly lower.
\begin{figure}[H]
\includegraphics[width=\textwidth]{ss-cpd-3.png}
\caption{Convergence rates for a Coherent Point Drift depending on rotation}
\end{figure}
